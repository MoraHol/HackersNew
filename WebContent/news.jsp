				...
				<td bgcolor="#ff6600"><table border="0" cellpadding="0"
						...
						<tr>
							...
							<td style="line-height: 12pt; height: 10px;"><span
								class="pagetop"><b class="hnname"><a href="newest">Hacker
				...
				<tr>
					<td><table border="0" cellpadding="0" cellspacing="0"
						...
						<%=URLEncoder.encode(notice.getTitle(), "utf-8")%>">web</a>
									| <%
										if (commentDao.findCommentsByNotice(notice.getId()).size() == 0) {
									%> <a href="item?id=<%=notice.getId()%>">discuss</a></td>
							</tr>
							<%
								} else {
							%>
							<a href="item?id=<%=notice.getId()%>"><%=commentDao.findCommentsByNotice(notice.getId()).size()%>
								comments</a>
							</td>
							</tr>
							<%
